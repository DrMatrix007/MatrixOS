...
#define _MATCH_OR_IMPL_4(TYPE, VAR, DATA, ON_EMPTY)                            \
    auto&& _MATCH_UNIQUE(_match_or_data) = (DATA);                             \
    auto&& _MATCH_UNIQUE(_match_or_data_view) =                                \
        (_MATCH_UNIQUE(_match_or_data)).view<TYPE>();                          \
    if (_MATCH_UNIQUE(_match_or_data_view).is_empty())                         \
    {                                                                          \
        return ([&]() { ON_EMPTY; })();                                        \
    }                                                                          \
    auto&& VAR = *(_MATCH_UNIQUE(_match_or_data_view).begin())

#define _MATCH_OR_IMPL_3(VAR, DATA, ON_EMPTY)                                  \
    auto&& _MATCH_UNIQUE(_match_or_data) = (DATA);                             \
    auto&& _MATCH_UNIQUE(_match_or_data_view) =                                \
        _MATCH_UNIQUE(_match_or_data).view();                                  \
    if (_MATCH_UNIQUE(_match_or_data_view).is_empty())                         \
    {                                                                          \
        return ([&]() { ON_EMPTY; })();                                        \
    }                                                                          \
